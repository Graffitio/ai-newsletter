...
### 4단계: 파일 업로드하기 (2번에 나눠서)

zip 파일 안에 보면 이런 파일들이 있습니다:
...
templates/logo.png
docs/  (빈 폴더)
github-actions-workflow/daily-briefing.yml   ← 이건 따로 올립니다
```

#### 4-A: 일반 파일 올리기

1. GitHub 저장소 페이지에서 **uploading an existing file** 링크 클릭
2. 아래 파일/폴더들을 드래그해서 올립니다:
   - `generate.py`
   - `requirements.txt`
   - `README.md`
   - `.gitignore`
...
#### 4-B: 자동 실행 설정 파일 올리기 (이게 핵심!)

GitHub에서 `.github` 으로 시작하는 폴더는 드래그로 안 올라갑니다.
대신 GitHub 웹에서 직접 만들어야 합니다:

1. 저장소 페이지에서 **Add file** → **Create new file** 클릭
2. 파일 이름 입력란에 아래를 **그대로** 입력합니다:
...
3. 아래 내용 편집 영역에, zip 안의 `github-actions-workflow/daily-briefing.yml` 파일을
   메모장으로 열어서 **내용을 전부 복사 → 붙여넣기** 합니다
4. 아래 **Commit changes** 클릭

> 💡 **확인**: 저장소에 `.github/workflows/daily-briefing.yml` 파일이 보이면 성공!
